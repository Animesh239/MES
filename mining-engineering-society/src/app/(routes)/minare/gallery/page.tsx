      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          const uniqueYearStrings = Array.from(
            new Set(data.map((img) => img.year).filter((y): y is string => !!y))
          );
    ...
    <div className="min-h-screen pt-24 pb-12 px-4 sm:px-6 lg:px-8 bg-black text-white">
      <div className="text-center mb-12 space-y-4">
        ...
        <p className="text-gray-400 text-lg max-w-2xl mx-auto">
          A visual journey through the history of Minare.
        </p>
      </div>

      {years.length === 0 ? (
        <div className="min-h-[50vh] flex flex-col items-center justify-center px-4">
          <Gallery uploadedImages={allImages} />
        </div>
      ) : (
        <>
          {/* Year Tabs */}
          <div className="flex flex-wrap justify-center gap-4 mb-16 px-4">
            ...
            {/* Option to view all images or images without year if needed, 
                for now we just show tabs for existing years */}
          </div>

          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 key={selectedYear}">
            <Gallery uploadedImages={filteredImages} />
          </div>
